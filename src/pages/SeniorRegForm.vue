<template>
    <q-page padding class="q-pa-md q-ma-md">
      <q-form
      @submit="onSubmit"
      @reset="onReset"
      class="q-gutter-md"
    >
      ...
      />
        ...
        lazy-rules
        :rules="[ val => val && val.length > 9 || 'Please put in a value here']"
      />
      ...
      />
        ...
        <q-input
        filled
        v-model="user.superintendentPharmName"
        label="Superintendent Pharm. Name "
        hint="Firstname and surname"
        autogrow
        lazy-rules
        :rules="[ val => val && val.length > 0 || 'This field is compulsory']"
      />

        <q-input
        filled
        v-model="user.superintendentPharmRegNo"
        label="Superintendent Pharm. Reg. No. "
        autogrow
        lazy-rules
        :rules="[ val => val && val.length > 0 || 'This field is compulsory']"
      />
        ...
        <q-input
        filled
        v-model="user.superintendentPharmLicenceNo"
        label="Superintendent Pharm. Licence No. "
        autogrow
      />
       ...
       <div>
        ...
        <q-btn no-caps flat rounded label="Clear" type="reset" color="primary" class="q-ml-sm" />
        </div>      
        </div>
       </div>
...
<script>
import User from 'pages/models/senior' ;
  ...
  data () {
    return {
      user: {
        ...
        company_name: '',
        practice_area: '',
        superintendentPharmName: '',
        superintendentPharmRegNo: '',
        superintendentPharmLicenceNo: ''
      },
      isPwd: true,
      message: ''
    }
  ...
  methods: {
    ...
    handleRegister() {
      ...
      let auth = new AuthService();
        auth.registerSen(this.user).then(
          response=> {
